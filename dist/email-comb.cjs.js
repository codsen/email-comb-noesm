...
function comb(str, originalOpts) {
  ...
  const finalIndexesToDelete = new rangesPush.Ranges({
    limitToBeAddedWhitespace: true
  });
  const currentChunksMinifiedSelectors = new rangesPush.Ranges();
  const lineBreaksToDelete = new rangesPush.Ranges();
  ...
  for (let round = 1; round <= 2; round++) {
    ...
    stepouter: for (let i = 0; i < len; i++) {
      ...
      if (!doNothing && (str[i] === '"' || str[i] === "'")) {
        if (!currentlyWithinQuotes) {
          const leftSideIdx = stringLeftRight.left(str, i);
      ...
      if (doNothing) {
        if (doNothingUntil === null || typeof doNothingUntil !== "string" || typeof doNothingUntil === "string" && !doNothingUntil) {
          doNothing = false;
        } else if (doNothingUntil && stringMatchLeftRight.matchRightIncl(str, i, doNothingUntil)) {
          if (commentStartedAt !== null) {
            if (round === 1 && opts.removeCSSComments) {
              const lineBreakPresentOnTheLeft = stringMatchLeftRight.matchLeft(str, commentStartedAt, ["\r\n", "\n", "\r"]);
      ...
      if (!doNothing && stateWithinStyleTag && str[i] === "@") {
        ...
        const matchedAtTagsName = stringMatchLeftRight.matchRight(str, i, atRulesWhichMightWrapStyles) || stringMatchLeftRight.matchRight(str, i, atRulesWhichNeedToBeIgnored);
      ...
      styleEndedAt && styleStartedAt > styleEndedAt && styleStartedAt <= i) && !insideCurlyBraces) {
        ...
        if (selectorChunkStartedAt === null) {
          ...
          if (round === 2 && selectorChunkCanBeDeleted && strArrToMatchAgainstChunks.length && matcher__default["default"]([currentChunk], strArrToMatchAgainstChunks).length) {
            selectorChunkCanBeDeleted = false;
          }
          if (round === 1) {
            if (whitespaceStartedAt) {
            ...
            const resToPush = stringRangeExpander.expander({
              str,
              from: fromIndex,
              to: toIndex,
      ...
      !str[i - 1].trim()) {
        ...
        if (str[i + 5] === "=") {
          if (str[i + 6] === '"' || str[i + 6] === "'") {
            ...
            const calculatedRange = stringRangeExpander.expander({
              str,
              from: i,
              to: i + 6,
              ifRightSideIncludesThisThenCropTightly: "/>",
      ...
      if (!doNothing && bodyClass.valuesStart !== null && i >= bodyClass.valuesStart && bodyClass.valueStart === null) {
        if (allHeads && stringMatchLeftRight.matchRightIncl(str, i, allHeads)) {
          ...
          const matchedHeads = stringMatchLeftRight.matchRightIncl(str, i, allHeads);
      ...
      if (!doNothing && bodyClass.valueStart !== null && i > bodyClass.valueStart && (!characterSuitableForNames(chr) || allTails && stringMatchLeftRight.matchRightIncl(str, i, allTails))) {
        if (allHeads && stringMatchLeftRight.matchRightIncl(str, i, allHeads)) {
          ...
          if (round === 1) {
            ...
            const expandedRange = stringRangeExpander.expander({
              str,
      ...
      if (!doNothing && bodyId && bodyId.valueStart !== null && i > bodyId.valueStart && (!characterSuitableForNames(chr) || allTails && stringMatchLeftRight.matchRightIncl(str, i, allTails))) {
      ...
      if (!doNothing && bodyClass.valuesStart != null && (!bodyClass.quoteless && (chr === "'" || chr === '"') || bodyClass.quoteless && !characterSuitableForNames(str[i])) && i >= bodyClass.valuesStart) {
        if (i === bodyClass.valuesStart) {
          if (round === 1) {
            finalIndexesToDelete.push(...stringRangeExpander.expander({
              str,
      ...
      if (commentNearlyStartedAt !== null && str[i] === ">") {
        ...
        if (opts.removeHTMLComments && Array.isArray(opts.doNotRemoveHTMLCommentsWhoseOpeningTagContains) && opts.doNotRemoveHTMLCommentsWhoseOpeningTagContains.length && (opts.doNotRemoveHTMLCommentsWhoseOpeningTagContains.some(val => val.includes("if")) || opts.doNotRemoveHTMLCommentsWhoseOpeningTagContains.some(val => val.includes("mso")) || opts.doNotRemoveHTMLCommentsWhoseOpeningTagContains.some(val => val.includes("ie"))) && stringMatchLeftRight.matchRight(str, i, "<!--", {
          ...
          if (stringMatchLeftRight.matchRight(str, temp - 1, "-->", {
            trimBeforeMatching: true,
            cb: (_char, _theRemainderOfTheString, index) => {
              temp = index;
              return true;
    ...
    if (round === 1) {
      ...
      headSelectorsArr.forEach(el => {
        stringExtractClassNames.extract(el).res.forEach(selector => {
      ...
      allClassesAndIdsWithinHead = uniq__default["default"](headSelectorsArr.reduce((arr, el) => arr.concat(stringExtractClassNames.extract(el).res), []));
      ...
      for (let y = 0, len2 = preppedHeadSelectorsArr.length; y < len2; y++) {
        ...
        if (temp && !temp.every(el => allClassesAndIdsWithinBody.includes(el))) {
          deletedFromHeadArr.push(...stringExtractClassNames.extract(preppedHeadSelectorsArr[y]).res);
      ...
      if (preppedHeadSelectorsArr?.length) {
        preppedAllClassesAndIdsWithinHead = preppedHeadSelectorsArr.reduce((acc, curr) => acc.concat(stringExtractClassNames.extract(curr).res), []);
      } else {
      ...
      bodyClassesToDelete = uniq__default["default"](bodyClassesToDelete.concat(intersection__default["default"](arrayPullAllWithGlob.pull(allClassesAndIdsWithinBody, opts.whitelist), allClassesAndIdsThatWereCompletelyDeletedFromHead).filter(val => val[0] === ".")
      .map(val => val.slice(1))));
      const allClassesAndIdsWithinBodyThatWereWhitelisted = matcher__default["default"](allClassesAndIdsWithinBody, opts.whitelist);
      bodyCssToDelete = uniq__default["default"](bodyCssToDelete.concat(bodyClassesToDelete.map(val => `.${val}`), bodyIdsToDelete.map(val => `#${val}`)));
      ...
      uglified = opts.uglify ? allClassesAndIdsWithinHeadFinal.map((name, id) => [name, allClassesAndIdsWithinHeadFinalUglified[id]]).filter(arr => !opts.whitelist.some(whitelistVal => matcher__default["default"].isMatch(arr[0], whitelistVal))) : null;
